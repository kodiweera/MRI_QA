...
for my $querynum (0..$#queries) {
  if ($querylanguages[$querynum] eq 'event' ||
      $querylanguages[$querynum] eq 'XCEDE') {
    for my $query ($queries[$querynum], $queryfilters[$querynum], $queryepochexcludes[$querynum]) {
      next if (!defined($query) || $query eq "");
      my $result = EventUtils::xcede_query_to_xpath($query);
      print STDERR "XCEDE query '$query' converted to XPath query '$result'\n" if ($opt_verbose);
      $query = $result;
    }
  }
}
if ($querylanguages[0] eq 'event' ||
    $querylanguages[0] eq 'XCEDE') {
  if (defined($opt_weightquery)) {
    my $result = EventUtils::xcede_query_to_xpath($opt_weightquery, 1, 0);
    print STDERR "XCEDE query '$opt_weightquery' converted to XPath query '$result'\n" if ($opt_verbose);
    $opt_weightquery = $result;
  } elsif (defined($opt_weightvaluequery)) {
    my $result = EventUtils::xcede_query_to_xpath($opt_weightvaluequery, 1, 1);
    print STDERR "XCEDE query '$opt_weightvaluequery' converted to XPath query '$result'\n" if ($opt_verbose);
